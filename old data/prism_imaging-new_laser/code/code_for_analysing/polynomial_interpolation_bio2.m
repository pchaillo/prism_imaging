function [carte_o, carte_x_o, carte_y_o, carte_time_o, bio_map_o] = polynomial_interpolation_bio2(carte,scan_pre,bio_map)
...
x = 0;
for i = carte.x(1 , 1 ) : scan_pas/scan_pre: carte.x( si(1) , 1 )% + scan.pas/scan.pre %%si-1
    x = x + 1;
    y = 0;
    for j = carte.y(1,1) : scan_pas/scan_pre : carte.y(1,si(2))% + scan.pas/scan.pre %%si-1
        y = y + 1;
        if carte_o(x,y) == 0
