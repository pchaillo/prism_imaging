...
function [bio_dat, time, g] = peak_detection_pt_par_pt_4(mzXMLStruct,threshold_begin,t_step,min_threshold,intern_flag,tolerance,carte_time)
...
[file, tab_peaks, ind_peaks2, ind_fusion] = peak_fusion(tab_peaks,tolerance,file,t_step);
...
file = collateral_fusion(file,ind_debut,ind_fin,min_threshold,ind_peaks2,ind_fusion,t_step);
...
for i = length(time_tab_espace) : -1 : 1 % suppression des points en trop
    ...
    elseif time_tab_espace(i) > t_step*0.7 % A FINIR
        i
...
% ind_f_new = reccur_time_recti(ind_f_new,file,time_tab_map,t_step);
 ind_f_new = reccur_time_recti_2(ind_f_new,file,time_tab_map,t_step,file_time_tab);
