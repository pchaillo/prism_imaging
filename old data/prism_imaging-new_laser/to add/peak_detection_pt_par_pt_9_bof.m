...
function [bio_dat, time, g] = peak_detection_pt_par_pt_9(mzXMLStruct,threshold_begin,t_step,min_threshold,intern_flag,tolerance,carte_time)
...
if si_time(1) ~= 1
    ...
    deep_flag = 0;
    [ind_f_new, ind_add_tab_2,end_deep_flag] = reccur_time_recti_4(ind_f_new,file,time_tab_map,t_step,file_time_tab,deep_flag);
 %  ind_add_tab_2 = ind_add_tab; 
...
%     end
    for i = 1 : length( time_tab_final_2)
        time_tab_diff_2(i) = time_tab_final_2(i) - time_tab_map(i);
    end
%     
    debug = 0;
    ...
    % Pour prendre le bon nombre de point pile pour la carte
    len = length(map_time);
    if len < length(ind_f_new)
