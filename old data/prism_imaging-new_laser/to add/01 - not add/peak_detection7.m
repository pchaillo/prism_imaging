...
function [bio_dat, time, g] = peak_detection7(mzXMLStruct,threshold_begin,t_step,min_threshold)

file_i = mzXMLStruct.scan ;

file = clean_time(file_i); % transformation du temps en une varibale numérique
file = clean_deiso(file);
...
ind_fusion =  find(tab_peaks(3,:) < t_step/3 ); % (t_step - t_step/6) bonne valeur
...
if deiso_ind > 0
    ...
    for i = 1 : length(indices_fusion)
        ...
        for j = 1 : length(tab_fusion)
            file(line_ind_max) = fusion_p1_top_B(file(tab_fusion(j)),file(line_ind_max),tab_fusion);
        end
...
[ind_communs_2 i_a2 i_b2 ] = intersect(ind_peaks_supp,ind_coll_dat_01);
...
for i = 1 : length(ind_coll_dat)
    if ind_coll_dat(i) < 0
        ...
        time_ind(p) = find_closest_pt_3(tps,peaks2_time_tab,t_step);
        p_ind_tab(p) = i;
    end
end
