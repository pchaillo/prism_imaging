function [ind_f_new, ind_add_tab_2 ,deep_flag] = reccur_time_recti_4(ind_f_old,file,time_tab_map,t_step,file_time_tab,deep_flag)
...
%utilisation de deep_flag pour savoir ce qui ne va pas
...
if length(time_tab_final_2) >= length(time_tab_map)
    ...
    for i = 1 : length( time_tab_map)
        ...
        if time_tab_diff_2(i) < - t_step*0.8
            if file(ind_f_old(i)).num < 0 % pour ne pas supprimer les lignes des peaks
                ind_f_old(i) = [];
                [ind_f_new, ind_add_tab_2,deep_flag]= reccur_time_recti_4(ind_f_old,file,time_tab_map,t_step,file_time_tab,deep_flag);
                ok = 1;
                break
            end
