function [carte_o, carte_x_o, carte_y_o, carte_time_o, bio_map_o] = polynomial_interpolation_bio3(carte,scan_pre,bio_map)

% Avec temps possiblement utilisé
...
si = size(carte.x);%-1; %% - 1 solution pas incroyable
...
x = 0;
for i = carte.x(1 , 1 ) : scan_pas/scan_pre: carte.x( si(1) , 1 ) %+ (scan_pas/scan_pre) %%si-1
    x = x + 1;
    y = 0;
    for j = carte.y(1,1) : scan_pas/scan_pre : carte.y(1,si(2)) %+( scan_pas/scan_pre) %%si-1
        y = y + 1;
        if carte_o(x,y) == 0
...
%% dimensions rectification
si_o = size(carte_o);
si_x = size(carte_x_o);

if si_o(1) ~= si_x(1) || si_o(2) ~= si_o(2)
    carte_o = carte_o(1:si_x(1),1:si_x(2));
    bio_map_o = bio_map_o(1:si_x(1),1:si_x(2));
end
