...
function [bio_dat, time, g] = peak_detection2(mzXMLStruct,threshold_begin,t_step)
...
% % end
% % ind_peaks2 = sort([tab(1,:),i_add2]); % mélange des indices pour que les points soient dans le bon ordre
% %
...
ind_peaks2 = tab(1,:); % indices des peaks detectés

% sup_deb = find(ind_peaks2 <= i_d-1);
% ind_peaks2(sup_deb) = []; % supprime les valeurs avant le premier point
...
for i = 1 : size_coll_fus(1)
    ...
    file(ind_f_m) = fusion_p1_coll(file(ind_f_m),file(ind_f_p));
end

bio_dat(:) = file(ind_final);

plot_peak3(bio_dat,t_i,ion);
