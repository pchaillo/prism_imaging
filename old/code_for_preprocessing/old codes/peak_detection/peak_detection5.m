...
function [bio_dat, time, g] = peak_detection5(mzXMLStruct,threshold_begin,t_step,min_threshold)
...
for i = 1 : length(ind_add)
    ...
    for j = 1 : nb_pt
        ...
        % if new_point_time < time_loc_tab(i) + time_diff_tab(i) - (t_step/3) % securite pour eviter la superposition de points
            new_point_ind = find_closest_pt_2(new_point_time,file_time_tab,t_step);
...
for i = 1 : length(ind_coll_dat)
    if ind_coll_dat(i) < 0
        ...
        time_ind(i) = find_closest_pt_2(tps,peaks2_time_tab,t_step);
    end
end
