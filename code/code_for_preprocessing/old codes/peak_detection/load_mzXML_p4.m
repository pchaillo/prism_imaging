...
function [bio_dat, time, g] = load_mzXML_p4(mzXMLStruct,threshold_begin,t_step)
...
ind_5 =  find(tab(3,:) > 2*t_step - t_step/10); % ajoute un point dans les grands espaces
...
for i = 1 :length(bio_dat)
    if floor(bio_dat(i).num) ~= bio_dat(i).num
        ind_bleu = ind_bleu + 1 ;
        pk_bleu(ind_bleu) = bio_dat(i).centroided;
        ...
        pk2(ind2) = bio_dat(i).totIonCurrent;
    end
end
plot(loc2,pk2,'*');
if exist('loc_bleu')
