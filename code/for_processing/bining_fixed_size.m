...
% masses qui nous interesse et step, la largeur de la fenetre de bining
...
function bined_peak_array = bining_fixed_size(peak_array,step,band)
...
for mz = band_begin : step : band_end - step
    
    all_peaks_over_mass_array = find ( peak_array(:,1) > mz );%&& peak_array(:,1) < mz +step);
    all_peaks_under_max_mass_array = find( peak_array(:,1) < mz + step);
    ...
    elseif min_ind == max_ind % if there is only one m/z that fit to the step in peak_aray
