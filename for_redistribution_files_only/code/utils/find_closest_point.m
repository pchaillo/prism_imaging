...
if isempty(begin_of_time_band_indices) || isempty(end_of_time_band_indices)
    ...
    if exist("gap_list")
        ...
        if length(indice_in_time_list) > 1
            update_log(app, log, 'Warning: Time is not properly recorded in the mzXML file. This may distort the peak detection');
            update_log(app, log, 'Attention: - Le temps est mal enregistré dans le fichier mzXML. Cela peut nuire à la détection de pics.');
            update_log(app, log, indice_in_time_list);
